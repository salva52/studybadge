<template>
	<div
		...
		:class="['grid grid-cols-1 lg:grid-cols-[2fr_1fr] xl:grid-cols-[5fr_3fr] gap-6 items-start', !showTitle && 'p-4 md:p-6']"
	>
		<div
		...
		<div class="flex flex-col space-y-6 lg:sticky lg:top-6">
			<div class="bg-white dark:bg-gray-800 shadow-sb-soft border border-gray-100 dark:border-gray-700 rounded-2xl p-6 space-y-5">
				...
				<!-- Chat con el Evaluador (Now stacked inside the right column) -->
				<div v-if="user.data?.name == submissionResource.doc?.owner && (chatHistory.length > 0 || submissionResource.doc?.comments)" class="bg-white dark:bg-gray-800 shadow-sb-soft border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden mt-6">
					<!-- Header -->
